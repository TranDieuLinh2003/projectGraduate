...
@Controller
@CrossOrigin("*")
@RequestMapping("/billTicket")
@Tag(name = "billTicket")
public class BillTicketController {
    @Autowired
    private BillTicketService service;
    ...
    public ResponseEntity<Object> save(@RequestBody @Valid BillTicket billTicket) {
        ResponseBean responseBean = new ResponseBean();
        responseBean.setCode(HttpStatus.OK.toString());
        responseBean.setMessage("SUCCESS");
        responseBean.setData(service.save(billTicket));
        return new ResponseEntity<>(responseBean, HttpStatus.OK);
    }

    @PutMapping("/update/{id}")
    @Operation(summary = "[Chỉnh sửa]")
    public ResponseEntity<Object> update(@PathVariable("id") String id, @RequestBody @Valid BillTicket billTicket) {
        ResponseBean responseBean = new ResponseBean();
        responseBean.setCode(HttpStatus.OK.toString());
        responseBean.setMessage("SUCCESS");
        responseBean.setData(service.update(id, billTicket));
        return new ResponseEntity<>(responseBean, HttpStatus.OK);
    }
